...
import com.dd.android.dailysimple.schedule.common.recycler.ViewHolder2.Companion.findViewById
import com.dd.android.dailysimple.schedule.common.recycler.ViewHolder2.Companion.isHeader
...
class DailyItemTouchAction(
    ...
    override fun onChildDraw(
        ...
        itemRoot.translationX = dX
        itemRoot.translationY = dY
        itemRoot.alpha = 1f - (abs(dX) / screenWidth)

        val rightSwipe = dX > 0

        swipeRoot.visibility = if (dX == 0f) View.GONE else View.VISIBLE
        swipeRoot.setChildVisibility(
        ...
        swipeRoot.setCardBackgroundColor(if (rightSwipe) doneColor else editColor)
        logD("dX : $dX, dY: $dY, actionState:$actionState, isActive:$isCurrentlyActive, alpha:${screenWidth}")
    }
